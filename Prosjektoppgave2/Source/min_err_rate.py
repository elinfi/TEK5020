...
def min_err_rate(test, train1, train2, train3):
    ...
    W3 = -1/2 * np.linalg.inv(cov3)

    w1 = np.linalg.inv(cov1) @ mu1.T
    w2 = np.linalg.inv(cov2) @ mu2.T
    w3 = np.linalg.inv(cov3) @ mu3.T

    w10 = -1/2 * mu1 @ np.linalg.inv(cov1) @ mu1.T \
          - 1/2 * np.log(np.linalg.det(cov1)) + np.log(P_omega1)
    w20 = -1/2 * mu2 @ np.linalg.inv(cov2) @ mu2.T \
          - 1/2 * np.log(np.linalg.det(cov2)) + np.log(P_omega2)
    w30 = -1/2 * mu3 @ np.linalg.inv(cov3) @ mu3.T \
          - 1/2 * np.log(np.linalg.det(cov3)) + np.log(P_omega3)

    segmentation = np.zeros((test.shape[0], test.shape[1], 3))
